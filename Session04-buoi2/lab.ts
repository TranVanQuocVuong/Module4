...
class User{
    ...
    constructor(email:string, password:string, userName:string, address:string){
        this.email = email;
    ...
    logIn(email:string, password:string){
        const checkLogIn = userArr.findIndex(
...
class Admin extends User{
    ...
    constructor(email:string, password:string, userName:string, address:string, id:string){
        super(email, password, userName, address)
        this.id = id;
    }

    addUser(id:string, email:string, password:string, userName:string, address:string){
        const newUser = {id, email, password, userName, address}
        const checkEmail = userArr.findIndex(
    ...
    updateUser(id:string, email:string, password:string, userName:string, address:string){
        const newUser = {id, email, password, userName, address}
        const checkUpdate = userArr.findIndex(
            (item) => item.id == id
        )
    ...
    deleteUser(id:string){
        const checkDelete = userArr.findIndex(
            (item) => item.id == id
        )
        if (checkDelete >= 0) {
